...
public struct RemoteDiagnosticRecord: Equatable, Sendable {
    ...
    private static let errorCodes: Set<String> = [
        ...
        "CLOUD_AUTH_FAILED", "PROVIDER_INSTALL_FAILED", "MPV_ERROR",
        ...
        "LIVE_CONTENT_REFRESH_FAILED", "LIVE_CONTENT_RECOVERY_FAILED",
    ...
    private static let breadcrumbCodes: Set<String> = [
        ...
        "MPV_CACHE_STALL", "MPV_TRACK", "MPV_TRACK_DETECTED",
    ...
    private static let measurementExpression = try! NSRegularExpression(
        pattern: #"(?:^|[\s,])(status|code|errorCode|attempt|elapsedMs|durationMs)=(-?[0-9]{1,9})(?=[\s,;]|$)"#
    )
    ...
    public init?(localMessage: String) {
        ...
        self.code = code
        self.isError = Self.errorCodes.contains(code)
